...
from utils import compute_accuracy, softmax


class CifarModel:
    def __init__(self, in_channel, lr=1e-4):
        self.lr = lr
        self.conv1 = cnn(in_channel=in_channel, out_channel=6, kernel_size=2, stride=1, padding=0)
        self.relu1 = ReLU()
    
        self.conv2 = cnn(in_channel=6, out_channel=16, kernel_size=2, stride=1, padding=0)
        self.relu2 = ReLU()

        self.fc1 = TwoLayerNetwork(14400, hidden_size=120, out_size=10)
